        ...
        <Reveal className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3.5 sm:grid-cols-3 lg:grid-cols-6">
            {PROFILE.map((p) => (
              <div
                key={p}
                className="rounded-xl border border-line bg-void-2/60 px-3.5 py-4 text-center text-[0.95rem] text-text transition-all duration-200 hover:-translate-y-[3px] hover:border-teal"
              >
                {p}
              </div>
            ))}
        ...
        <div className="grid gap-5 lg:grid-cols-2" style={{ perspective: 1400 }}>
          <Reveal className="overflow-hidden rounded-2xl border border-line bg-void-3">
            <div className="flex items-center justify-between border-b border-line px-[18px] py-3">
              <span className="font-mono text-[12px] tracking-[0.15em] text-teal">
                protein_target.rule.yaml
              </span>
              <span className="font-mono text-[11px] tracking-[0.15em] text-text-dim/60">
                Rules engine
              </span>
            </div>
            <div className="flex flex-col px-[22px] py-5 font-mono text-[14px] leading-[1.8] text-text">
          ...
          <div className="flex flex-col gap-5">
            <Reveal delay={0.08}>
              <TiltCard max={7} className="hud-corners rounded-2xl border border-line bg-void-2/70 px-6 py-[22px] backdrop-blur-md">
                <div className="mb-[18px] flex flex-wrap items-center justify-between gap-2">
                  ...
                  <span className="rounded-full border border-brand-lime/40 px-2.5 py-1 font-mono text-[11px] uppercase tracking-[0.15em] text-lime">
                    Clinician approved
                  </span>
                </div>
            ...
            <Reveal delay={0.16} className="flex flex-col gap-3 rounded-2xl border border-line bg-void-2/70 px-6 py-[22px] backdrop-blur-md">
              ...
              {GUARDRAILS.map((g) => (
                <div key={g} className="flex items-baseline gap-2.5 text-[1rem] leading-[1.55] text-text/75">
